...
# Oracle Cloud Always Free Tier Bootstrap Script
# This script sets up an ARM A1 instance for your car platform backend

set -e

echo "🚀 Starting Oracle Cloud Always Free Tier setup for Car Platform Backend"
...
if docker-compose -f docker-compose.oracle.yml ps postgres | grep -q "Up"; then
    docker-compose -f docker-compose.oracle.yml exec -T postgres pg_dump -U $DB_USER $DB_NAME > $BACKUP_DIR/db_backup_$DATE.sql
fi
...
echo "3. Run: cd /opt/car-platform && docker-compose -f docker-compose.oracle.yml up -d"
