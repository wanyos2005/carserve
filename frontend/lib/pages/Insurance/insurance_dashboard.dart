...
class _InsuranceDashboardState extends State<InsuranceDashboard> {
  ...
  Future<void> _loadInsuranceData() async {
    ...
    try {
      ...
      // Ensure risk score exists; if not, calculate it once
      Map<String, dynamic>? ensuredRiskScoreData = results[2] as Map<String, dynamic>?;
      if (ensuredRiskScoreData == null || ensuredRiskScoreData.isEmpty) {
        try {
          final calculated = await InsuranceService.calculateRiskScore(
            vehicleId: 'mock-vehicle',
            userId: userId,
          );
          if (calculated != null && calculated.isNotEmpty) {
            ensuredRiskScoreData = calculated;
          }
        } catch (_) {
          // Ignore and proceed with null risk score
        }
      }
      ...
      setState(() {
        ...
        _riskScore = (ensuredRiskScoreData != null && ensuredRiskScoreData.isNotEmpty)
            ? RiskScore.fromJson(ensuredRiskScoreData)
            : null;
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => CInsurancePolicyPage(),
                          ),
                        );
                      },
