...
class _CreatePostPageState extends State<CreatePostPage> {
  ...
  Future<void> _pickVideo() async {
    try {
      ...
      if (video != null) {
        ...
        await _uploadMedia(File(video.path));
      }
    } catch (e) {
  ...
  Future<void> _uploadMedia(File file) async {
    ...
    try {
      final url = await SocialService.uploadMedia(file, folder: 'posts');
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  Icon(
                    _mediaUrls.isNotEmpty ? Icons.image : Icons.text_fields,
                    color: _mediaUrls.isNotEmpty ? Colors.green[700] : Colors.orange[700],
                  ),
                  const SizedBox(width: 8),
                  Text(
                    _mediaUrls.isNotEmpty 
                        ? 'Media Post (${_mediaUrls.length} file${_mediaUrls.length > 1 ? 's' : ''})'
                        : 'Text Post',
                    style: TextStyle(
                      fontWeight: FontWeight.w600,
                      color: _mediaUrls.isNotEmpty ? Colors.green[700] : Colors.orange[700],
